...
angular.module("acute.select", ['angularUUID2'])
...
.directive("acSelect", ['$parse', 'AcuteSelectService', 'uuid2', 
                        function($parse, acuteSelectService, uuid2) {
    ...
    return {
        ...
        link: function(scope, element, attrs) {
        	scope.directiveID = uuid2.newuuid();        	
            scope.initialise();
        },
        // **************************************************************
        //                          CONTROLLER
...
.controller('AcSelectController', ['$scope', '$element', '$window', '$rootScope', '$timeout', '$filter', 'navKey', 'safeApply', 'AcuteSelectService',
                                   function($scope, $element, $window, $rootScope, $timeout, $filter, navKey, safeApply, acuteSelectService){
	
	var dataFunctionFiltering = false;
    
    // fix for this directive to re-initialize
    $scope.$on('acute.select.event.refresh', function(event, args){
    	$scope.initialise();
    });
    ...
    $scope.comboTextChange = function() {
    	$rootScope.$broadcast('acute.select.event.text.changed', {text:$scope.comboText});
...
.controller('AcScrollToController', ['$scope', '$element', '$attrs', function($scope, $element, $attrs){
